...
COPY bin/docker-custom-ext-install-apache /usr/local/bin/

# Install gRPC separately.
RUN docker-custom-ext-install-apache grpc-1.38.0

# Install the other extensions.
RUN set -ex; \
    docker-custom-ext-install-apache \
        amqp-1.11.0beta \
        bcmath \
        bz2 \
        exif \
...
RUN set -ex; \
    ...
    apt-get clean; \
    sed -i 's/libphp7.so/libphp.so/' /etc/apache2/mods-available/php.load; \
    mkdir -p /docker-entrypoint.d /var/run/apache2; \
    rm -rf /etc/apache2/sites-*; \
    a2disconf '*'; \
    a2dismod mpm_*; \
    a2enmod mpm_prefork headers php
